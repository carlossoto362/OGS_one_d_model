...
def run_inversion(x_data_tensor_T,perturbation_factors,rank,world_size,initial_conditions_path=None):
    """
    ...
    for x_data_tensor in x_data_tensor_T:
        x_result[job] = run_one_inversion(job,x_data_tensor,perturbation_factors,initial_conditions_path=initial_conditions_path)
        job+=1
    return x_result
...
def run_one_inversion(x_index,x_data_tensor,perturbation_factors,initial_conditions_path=None):
        """
	...
	run_inversion.
        ...
        state_dict['chparam'] = torch.ones((3), dtype=torch.float32)*torch.tensor([x_data['chla'].iloc[0],x_data['NAP'].iloc[0],x_data['CDOM'].iloc[0]])#initial conditions equal to the first run (with the base parameters)
        ...
        chla = list(model.parameters())[0][1]
        ...
        if initial_conditions_path == None:
            ...
            np.save(initial_conditions_path+'/'+str(int(x_data_tensor[-1].clone().detach().numpy())),\
                    np.array([chla.clone().detach().numpy(),nap.clone().detach().numpy(),cdom.clone().detach().numpy()]))
...
class customTensorData():
    ...
    def __init__(self, initial_conditions_path,data_path,which='Train', transform=None, target_transform=None,rank=0,initial_conditions_path_created=False,past_initial_conditions_path_index=0):
        ...
        self.initial_conditions_init = self.images[:,22:25]

        self.images = np.delete(self.images,[22,23,24],axis=1)#removing chla_init,nap_init,cdom_init
        ...
        self.initial_conditions_path = initial_conditions_path
	...
	col1,col2,col3,col4,col5,col6,col7,col8,col9
        ...
        initial_conditions = np.load(self.initial_conditions_path + '/'+ str(int(self.images[idx][-1]))+'.npy')
...
class Parameter_Estimator(nn.Module):
    ...
    def forward(self,image,rank,world_size,initial_conditions_path=None):
        """
        x_data: pandas dataframe with columns [E_dif,E_dir,lambda,zenith,PAR].
        """

        return run_inversion(image,self.perturbation_factors,rank,world_size,initial_conditions_path=initial_conditions_path)
...
def train_loop_parameters(dataloader, model_par, loss_fn_par, optimizer_par,scheduler_par,labels_stds,rank,world_size,batch_size,learning_rate,initial_conditions_path=None):
    """
...
def parallel_training(rank,world_size,batch_size,learning_rate):
    ...
    initial_conditions_path = '/Users/carlos/Documents/surface_data_analisis/npy_data/initial_conditions'
    ...
    for t in range(epochs):
        if rank == 0:
            print("Epoch: ",t)
        train_loop_parameters(train_dataloader,model_par,loss_fn_par,optimizer_par,scheduler_par,labels_stds,rank,world_size,batch_size,learning_rate,initial_conditions_path = initial_conditions_path)
        test_loop_parameters(test_dataloader, model_par, loss_fn_par,labels_stds,rank,world_size,batch_size,learning_rate)
    if rank == boss:
