...
class composed_loss_function(nn.Module):
    ...
    def forward(self,pred_,Y_obs,rrs_,rrs_pred,nan_array, param):
        ...
        rrs_error = torch.trace(   (rrs_ - rrs_pred) @ ( self.rrs_cov_inv @ (rrs_ - rrs_pred ).T ) )/5
        
        lens = torch.tensor([len(element[~element.isnan()])  for element in nan_array])
        obs_error = torch.trace(   ((pred_ - Y_obs) @ ( self.Y_cov_inv @ (pred_ - Y_obs ).T ))/lens)

        return rrs_error + obs_error #+ 0.00001*param.mean()
...
def get_training_history(dataloader,test_dataloader,model,loss_fn,optimizer,epochs,batch_size, save = False, save_path = None):
    ...
    if save == True:
        np.save(save_path + '/train_loss_VAE.npy',train_loss)
        np.save(save_path + '/test_loss_VAE.npy',test_loss)
        np.save(save_path + '/chla_history_VAE.npy',chla_history)
        np.save(save_path + '/kd_history_VAE.npy',kd_history)
        np.save(save_path + '/bbp_history_VAE.npy',bbp_history)
        np.save(save_path + '/perturbation_factors_history_VAE.npy',perturbation_factors_history)
...
if __name__ == '__main__':
    ...
    torch.save(data.x_mean,'/Users/carlos/Documents/surface_data_analisis/LastVersion/VAE_model/x_mean.npy')
    torch.save(data.y_mean,'/Users/carlos/Documents/surface_data_analisis/LastVersion/VAE_model/y_mean.npy')
    torch.save(data.x_std,'/Users/carlos/Documents/surface_data_analisis/LastVersion/VAE_model/x_std.npy')
    torch.save(data.y_std,'/Users/carlos/Documents/surface_data_analisis/LastVersion/VAE_model/y_std.npy')
    ...
    torch.save(model.state_dict(),'/Users/carlos/Documents/surface_data_analisis/LastVersion/plot_data/model_state_dict_VAE.npy')
    ...
    model.load_state_dict(torch.load(load_path + '/model_state_dict_VAE.npy'))
    model.eval()
    ...
    indexes = data.dates>=(datetime(year=2012,month=1,day=1) -  datetime(year=2005,month=1,day=1)).days

    plt.plot(data.dates[indexes],data.y_data[indexes,0],label='data')
    plt.plot(data.dates[indexes],chla_hat_mean[indexes,0].clone().detach(),label='model vae')
    plt.legend()
    plt.show()
    ...
    def save_var_uncertainties(model, X, chla_hat_mean, covariance_matrix,rrs_hat, constant = None, save_path ='/Users/carlos/Documents/surface_data_analisis/LastVersion/results_VAE_VAEparam',dates = [] ):
